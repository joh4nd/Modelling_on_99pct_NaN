...
for sample_no, filename in enumerate(public_list, start=1):
    ...
    rebs_df['ship_sample'] = rebs_df.apply(lambda df_x: pd.isna if pd.isna(df_x['ship']) else str(int(df_x['ship']))+'_'+str(df_x['sample']),axis=1)

    # inspect
    print(rebs_df.info())
    print('What type of leaks are most common? (are they?)')
