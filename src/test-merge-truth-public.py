...
p_info = rd.parse_public_data("../data/0003_public.txt")
...
if LOC['ship'].isna().sum() < 1:
    ...
    LOC2 = LOC.loc[LOC['ship'].notna()]

    ################################################################
    # We see that leakers aboard same ships like slightly different LOC at t
    # LOC2.loc[LOC2[['ship','t']].duplicated(subset=['ship','t'],keep=False)].sort_values(['ship','t'])
    ...
    if LOC2[['ship','t','x']].duplicated(subset=['ship','t']).sum() > 0:
        ...
        LOC2 = LOC2[['ship','t','x','y','z']].drop_duplicates().sort_values(by=['ship','t'])

    rebs_df = rebs_df.combine_first(rebs_df.drop(['x','y','z'],axis=1).merge(LOC2, how='left',on=['ship','t']))
