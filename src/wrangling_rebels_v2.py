...
def wr_2(directory_to_files="../data/*.txt"):
    ...
    for sample_no in sample_set:
        ...
        rebs_df['ship_sample'] = rebs_df.apply(lambda df_x: np.nan if pd.isna(df_x['ship']) else str(df_x['ship'])+df_x['sampleNo'],axis=1) # make ships unique across samples to imply/convey no cross-sample information
        ...
        rebs_df.rename(columns={'ship': 'shipNo', 'ship_sample': 'shipId','closestStar':'closestStarId','id': 'messengerId_truth', 'shipid': 'shipId_truth','at_dest':'atDest_moving'},inplace=True)
